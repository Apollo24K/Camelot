...
module.exports = {
	...
	execute(interaction) {
        ...
        db.serialize(async () => {
            var inv = await query(`SELECT users.premium, characters.chars, characters.level FROM characters JOIN users ON characters.id = users.id WHERE characters.id = ${interaction.user.id}`);
            ...
            interaction.reply({content: `Do you want to reset **${char.name}**'s level for **${price}**<:coins:872926669055356939>? (You will get ${rPer*100}% back of what you've invested)`, components: [row], fetchReply: true}).then(msg => {
                ...
                confirm.on('collect', async r => {
                    ...
                    var checkLevel = await query(`SELECT level FROM characters WHERE id = ${interaction.user.id}`);
