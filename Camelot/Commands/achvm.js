        ...
        db.serialize(async () => {
            ...
            function progress(id) {
                switch (id) {
                  ...
                  case 27: 
                  case 28: 
                  case 29: let bossFloors = [], wins = 0;
                           enemies.forEach((e) => e.boss ? bossFloors.push(e.floor[0]) : false);
                           if (stats.floors) wins = Math.max(...new Set(bossFloors.map((e) => stats.floors[e] ? stats.floors[e] : 0)));
                  ...
                  case 36: 
                  case 37: 
                  case 38: let cFloor = 0;
                           if (Object.entries(stats.floors).reverse()[0][1]) cFloor = Object.entries(stats.floors).reverse()[0][0];
                           else if (Object.entries(stats.floors).length > 1) cFloor = Object.entries(stats.floors).reverse()[1][0];
                  ...
                  case 42: return achvmBar(Math.max(1, ...Object.values(JSON.parse(stats.charlvl)))/30, ` (${Math.max(...Object.values(JSON.parse(stats.charlvl)))}/30)\n`);
                  case 43: return achvmBar(Math.max(1, ...Object.values(JSON.parse(stats.charlvl)))/50, ` (${Math.max(...Object.values(JSON.parse(stats.charlvl)))}/50)\n`);
                  case 44: return achvmBar(Math.max(1, ...Object.values(JSON.parse(stats.charlvl)))/80, ` (${Math.max(...Object.values(JSON.parse(stats.charlvl)))}/80)\n`);
                  case 45: return achvmBar(Math.max(1, ...Object.values(JSON.parse(stats.charlvl)))/100, ` (${Math.max(...Object.values(JSON.parse(stats.charlvl)))}/100)\n`);
                  case 46: 
                  case 47: 
                  case 48: 
                  case 49: return achvmBar(0);
            ...
            function rew(a) {
                ...
                a.type.forEach((type) => {
                    switch (type) {
                        ...
                        case "3": a.rewards.forEach((rew) => { if (rew.match(/shard/gi)) rews.push(`**${rew.split("|")[1]}**x ${shardEmojis[rew.split(" ")[0]]}`); }); break;
                        case "4": a.rewards.forEach((rew) => { if (rew.match(/ticket/gi)) rews.push(`**${rew.split("|")[1]}**x ${ticketEmojis[rew.split(" ")[0]]}`); }); break;
                        case "5": a.rewards.forEach((rew) => { if (rew.match(/lb/gi)) rews.push(`**${rew.split("|")[1]}** ${rew.split("|")[1] == "1" ? "lootbox" : "lootboxes"}`); }); break;
                    };
                });
                let ret = rews[0];
                for (j=1; j < rews.length; j++) {
                    if (j%2 !== 0) ret += `, ${rews[j]}`;
                    else ret += `,\n${rews[j]}`;
                };
                return ret;
            ...
            for (i=0; i <= achievements[achievements.length-1].group; i++) {
                let achvms = achievements.filter((e) => e.group === i);
            ...
            function updatePage() {
                ...
                if (currPage < pagesTotal || left === 0) {
                    for (i=(currPage-1)*fieldPerPage; i < currPage * fieldPerPage; i++) {
                        Embed.addFields(fields[i][0], fields[i][1], fields[i][2]);
                    };
                } else {
                    for (i=(currPage-1)*fieldPerPage; i < (currPage * fieldPerPage) - (fieldPerPage-left); i++) {
                        Embed.addFields(fields[i][0], fields[i][1], fields[i][2]);
                    };
                };
                return Embed;
