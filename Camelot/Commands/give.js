...
module.exports = {
	...
	execute(interaction) {
        ...
        db.serialize(async () => {
            var _stats = await query(`SELECT coins FROM users WHERE id = ${user.id}`);
            ...
            if (subcommand === "coins") {
                ...
                return interaction.reply({content: `Are you sure you want to give **${user.username}** **${amount}**<:coins:872926669055356939>?`, components: [row], fetchReply: true }).then(msg => {
                    ...
                    confirm.on('collect', async r => {
                        ...
                        interaction.channel.send(`Sent **${amount}**<:coins:872926669055356939> to **${user.username}**`);
            ...
            if (subcommand === "character") {
                ...
                return interaction.reply({content: `Are you sure you want to give **${char.name}** to **${user.username}**?`, components: [row], fetchReply: true}).then(msg => {
                    ...
                    confirm.on('collect', async r => {
                        ...
                        var _inv = await query(`SELECT chars FROM characters WHERE id = ${user.id}`);
                        ...
                        interaction.channel.send(`**${char.name}** was gifted to **${user.username}**`);
                        confirm.stop(), cancel.stop();
            ...
            if (subcommand === "premium") {
                ...
                switch (stats[0].premium) {
                    ...
                    case 7: giftLimit = 3, giftTier = 1; break;
                    default : false; break;
                };
                ...
                if (stats[1].premium) return interaction.reply(`**${user.username}** already has premium.`);
