...
class achievInfo {
    ...
    check(interaction, user=undefined, ...list) {
        ...
        db.serialize(async () => {
            var stats = await query(`SELECT achievements, pullstotal, arenawins, xp FROM users WHERE id = ${user.id}`);
            stats = stats[0];
            if (!stats) return;
            ...
            switch(this._id) {
                ...
                case 4: if (list[0] < 21 && list[0] > 2) this.addRewards(interaction, user), this.notify(interaction); break;
                case 5: if (list[0] < 3) this.addRewards(interaction, user), this.notify(interaction); break;
                ...
                case 23: let completed = 0;
                         ...
                         if (this._id === 23) if (completed >= 250) this.addRewards(interaction, user), this.notify(interaction); break;
...
const achievements = [ // Type 1: xp, 2: coins, 3: shards, 4: tickets, 5: lootbox
    ...
    new achievInfo("Shared Happiness", "Gift someone an SS tier character immediately after pulling it", 32, 10, "1,4", "xp|60", "ss ticket|1", "s ticket|2"),
