...
module.exports = {
    ...
    execute(interaction) {
        ...
        db.serialize(async () => {
            ...
            if (inv[shardRarity + "shard"] < shardAmount) return interaction.reply(`You don't have enough shards (**${inv[shardRarity + "shard"]}**/${shardAmount}${shardStr})`);
            ...
            interaction.reply({ embeds: [Embed], components: [OfferRow], fetchReply: true }).then(msg => {
                ...
                confirm.on('collect', async () => {
                    ...
                    if (invCheck[shardRarity + "shard"] < shardAmount) return interaction.channel.send(`You don't have enough shards (**${invCheck[shardRarity + "shard"]}**/${shardAmount}${shardStr})`);
                    if (invCheck.coins < price) return interaction.channel.send(`You don't have enough coins (**${invCheck.coins}**/${price})`);

                    if (!invCheck.ref[char.id]) invCheck.ref[char.id] = 0;
                    invCheck.ref[char.id]++;

                    await query(`UPDATE users SET coins = coins - ${price}, ${shardRarity + "shard"} = ${shardRarity + "shard"} - ${shardAmount} WHERE id = ${interaction.user.id}`);
                    await query(`UPDATE characters SET ref = '${JSON.stringify(invCheck.ref)}' WHERE id = ${interaction.user.id}`);
