...
const { characters, charactersA } = require("../Modules/chars.js");
	...
	execute(interaction) {
        ...
        db.serialize(async () => {
            ...
            async function triggerTutorial() {
                let stats = await query(`SELECT users.tutorial, users.favchar, users.battlechar, users.premium, users.items, characters.chars, characters.ref, characters.class, characters.skin, characters.equipment, dungeon.classes FROM users JOIN characters ON users.id = characters.id JOIN dungeon ON dungeon.id = users.id WHERE users.id = ${interaction.user.id}`);
                ...
                if (tutorial === 0) {
                    ...
                    interaction.editReply({ embeds: [Embed], components: [row], fetchReply: true }).then((msg) => {
                        ...
                        const collector = msg.createMessageComponentCollector({filter: (r) => r.user.id === interaction.user.id && r.customId === "accept", componentType: 'BUTTON', time: 120000 });
                    ...
                    interaction.editReply({ content: "Hey, look! You got an<:ATier:869316558013464627>-Tier character, they're quite rare!", embeds: [Embed], components: [row], fetchReply: true }).then((msg) => {
                        ...
                        const collector = msg.createMessageComponentCollector({filter: (r) => r.user.id === interaction.user.id && r.customId === "continue", componentType: 'BUTTON', time: 60000 });
                    ...
                    .setThumbnail("https://i.imgur.com/Ta2YDBN.png")
                    .setDescription(`You will be able to pull **6** characters evey **45** minutes, and there's more ways to get them such as through the \`/shop\`, \`/tickets\` or \`/lootbox\`.\n\nCharacters come in the rarities of\n<:DTier:869316616071032843>➜<:CTier:869316602858991657>➜<:BTier:869316586803179571>➜<:ATier:869316558013464627>➜<:STier:869316518675095552>➜<:SSTier:869316489931546644>\n\nYou can view your characters with \`/inventory\` or \`/info\` if you wanna see details on a specific character.\n\n**Tip**: I'm also available as a pullable character <:LuminousPsssh:1071574041116295328>`)
                    ...
                    if (stats.classes.length) {
                        ...
                        if (interaction.commandName === "class" && interaction.options.getSubcommand() === "info") {
                            ...
                            return interaction.editReply({ embeds: [Embed], components: [row], fetchReply: true }).then((msg) => {
                                ...
                                const collector = msg.createMessageComponentCollector({filter: (r) => r.user.id === interaction.user.id && r.customId === "class_selection", componentType: 'SELECT_MENU', time: 120000 });
                    ...
                    interaction.editReply({ embeds: [Embed], components: [row], fetchReply: true }).then((msg) => {
                        ...
                        collector.on('collect', async r => {
                            ...
                            // Assign weapon
                            stats.equipment[stats.battlechar] = {weapon: uid + ":" + interaction.user.id};
