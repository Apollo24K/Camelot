...
function participationPrize(rounds) {
    const prizePool = {
...
function hoursToNextThird() {
    ...
    const diffHours = Math.floor((nextThird - now) / (1000 * 60 * 60));

    if (diffHours === 0) return `${Math.floor(((nextThird - now) % (1000 * 60 * 60)) / (1000 * 60))}min left`;
        ...
        db.serialize(async () => {
            ...
            if (boss === 1) {
                ...
                if (requestVerification.has(interaction.user.id)) {
                    ...
                    await query(`UPDATE dungeon SET s_responsetime = s_responsetime || '${dateNow},${dateNow},' WHERE id = ${interaction.user.id}`);
            ...
            async function matchResult(r) {
                ...
                (party ? party.members.split(",") : [interaction.user.id]).forEach((e) => {
                    if (damages.participation[e]) damages.participation[e][0] += damageDealt;
                    else damages.participation[e] = [damageDealt, 0];
                });
                damages.participation[interaction.user.id][1] += (enemyType === "monster" ? 1 : 5);
                ...
                if (enemyType === "monster") {
                    await query(`UPDATE stampedes SET ${r === "w" ? "monsterstotal = monsterstotal + 1, " : ""}participation = '${JSON.stringify(damages.participation)}' WHERE rowid = ${stampede.rowid}`);
                } else if (enemyType === "general") {
