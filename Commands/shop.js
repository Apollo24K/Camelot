...
module.exports = {
	...
	execute(interaction) {
        ...
        db.serialize(async () => {
            let stats = await query(`SELECT coins, gems, genesispity FROM users WHERE id = ${interaction.user.id}`);
            stats = stats[0];
            if (!stats) stats = {coins: 0, gems: 0, genesispity: 0};
