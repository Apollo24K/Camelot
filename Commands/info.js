...
module.exports = {
    ...
    execute(interaction) {
        ...
        const flag = interaction.options.getString('flag') ?? (choice ? "base" : "detailed");

        db.serialize(async () => {
            const { 0: stats } = await query(`SELECT battlechar, premium, skins, class, level, bank, equipment, shield_slot FROM users WHERE id = ${user.id}`);
            ...
            let inv = await query(`SELECT characters.id, characters.chars, characters.ref, characters.skin, dungeon.classes, dungeon.classlevels FROM characters JOIN dungeon ON characters.id = dungeon.id WHERE characters.id = ${user.id}`);
            inv = { id: user.id, class: stats.class, premium: stats.premium, bank: stats.bank, shield_slot: stats.shield_slot, equipment: JSON.parse(stats.equipment), chars: JSON.parse(inv[0].chars), ref: JSON.parse(inv[0].ref), level: stats.level, skin: JSON.parse(inv[0].skin), classes: JSON.parse(inv[0].classes), classlevels: JSON.parse(inv[0].classlevels) };
            ...
            if (flag === "base") {
                ...
                let fSkins, currentSkin, pagesTotal;
                let hp, atk, def, ep, expertise;
                ...
                return interaction.reply({ embeds, components: [PageRow], fetchReply: true }).then(msg => {
            ...
            if (flag === "my") {
                ...
                for (const char of chars) {
                    ...
                    charstats = await getDetailedStats(char.id, inv, inv.classlevels);
                    cls = charstats.class === -1 ? "None" : `${classes[charstats.class].name}${classes[charstats.class].emblem}Lvl. ${charstats.clvl}`;
                    dupes = inv.chars.filter((e) => e === char.id).length;

                    img = char.getImage(stats.premium, customSettings[user.id]?.cimg[char.id], inv.skin[char.id]);
                    if (char.id === 18011 && charstats.maskinfo) img = { "phantasmal": "https://i.imgur.com/vKmnIqq.png", "verdant": "https://i.imgur.com/sWYC62u.png", "valkyrie": "https://i.imgur.com/Sn3MQZ7.png" }[charstats.maskinfo];
                    ...
                    const Embed = new EmbedBuilder()
                        ...
                        .setFooter({ text: `You have ${dupes} ${dupes === 1 ? "copy" : "copies"} of ${char.gender === "F" ? "her" : "him"}\nEP: ${charstats.ep} | Expertise: ${charstats.expertise}`, iconURL: user.displayAvatarURL({ dynamic: true }) + "?size=2048" });
                    // if (skins.filter((e) => e.cid == char.id).length === 0) return interaction.reply({ embeds: [Embed] });

                    if (skins.filter((e) => e.cid == char.id).length > 0) {
                        fSkins = [{ image: img, name: char.name }, ...skins.filter((e) => e.cid == char.id)];
                        currentSkin = fSkins.indexOf(fSkins.find((e) => e.id === inv.skin[char.id])) + 1 || 1;
                        pagesTotal = fSkins.length;
                ...
                if (embeds.length === 0) return interaction.reply(`No match found`);
                if (embeds.length > 1 || !pagesTotal) return interaction.reply({ embeds });
                return interaction.reply({ embeds, components: [row], fetchReply: true }).then(msg => {
                    ...
                    select.on('collect', async () => {
                        if (currentSkin === 1 || stats.skins.includes(fSkins[currentSkin - 1].id)) {
                            if (currentSkin === 1) {
                                delete inv.skin[chars[0].id];
                            } else {
                                inv.skin[chars[0].id] = fSkins[currentSkin - 1].id;
                            };

                            await query(`UPDATE characters SET skin = '${JSON.stringify(inv.skin)}' WHERE id = ${user.id}`);

                            interaction.channel.send(`Set **${chars[0].name}**'s skin to **${fSkins[currentSkin - 1].name}**`);
            ...
            if (flag === "detailed") {
                ...
                for (const char of chars) {
                    ...
                    if (!inv.chars.includes(char.id)) return interaction.reply(`You don't have a copy of **${char.name}**`);
                    let charstats = await getDetailedStats(char.id, inv, inv.classlevels);
