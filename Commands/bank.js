...
module.exports = {
    ...
    execute(interaction) {
        ...
        db.serialize(async () => {
            const { 0: stats } = await query(`SELECT coins, bank, level, favchar, premium FROM users WHERE id = ${user.id}`);
            ...
            if (subcommand === "view") {
                ...
                return interaction.reply({ embeds: [Embed], fetchReply: true }).then(() => {
                    if (stats.bank === -1 && user.id === interaction.user.id) {
                        ...
                        interaction.followUp({ embeds: [Embed], components: [row], fetchReply: true, ephemeral: true }).then((msg) => {
                            ...
                            collector.on('collect', () => {
                                collector.stop();
                                db.serialize(async () => {
                                    await query(`UPDATE users SET bank = 0 WHERE id = ${interaction.user.id} AND bank = -1`);
                                });
                            });
                        });
                    };
