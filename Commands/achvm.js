...
module.exports = {
    ...
    execute(interaction) {
        ...
        db.serialize(async () => {
            const { 0: stats } = await query(`SELECT users.favchar, users.dailystreak, users.xp, users.premium, users.arenawins, users.achievements, characters.chars, users.level AS charlvl, characters.skin, dungeon.floors FROM users JOIN characters ON users.id = characters.id JOIN dungeon ON users.id = dungeon.id WHERE users.id = ${user.id}`);
            ...
            function progress(id) {
                switch (id) {
                    ...
                    case 42: return achvmBar(stats.charlvl / 30, ` (${stats.charlvl}/30)\n`);
                    case 43: return achvmBar(stats.charlvl / 50, ` (${stats.charlvl}/50)\n`);
                    case 44: return achvmBar(stats.charlvl / 80, ` (${stats.charlvl}/80)\n`);
                    case 45: return achvmBar(stats.charlvl / 100, ` (${stats.charlvl}/100)\n`);
                    case 46:
                    case 47:
                    case 48:
                    case 49:
