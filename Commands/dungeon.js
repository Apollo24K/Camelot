...
module.exports = {
    ...
    execute(interaction) {
        ...
        db.serialize(async () => {
            ...
            // Captcha
            if (!requestVerification.has(interaction.user.id) && stats.limit > 100 && Math.random() < 0.005 && !captchaCooldown.has(interaction.user.id)) requestVerification.set(interaction.user.id, { repeats: 0 });
            if (requestVerification.has(interaction.user.id)) {
                ...
                await query(`UPDATE dungeon SET responsetime = responsetime || '${dateNow},${dateNow},' WHERE id = ${interaction.user.id}`);
            ...
            await query(`UPDATE dungeon SET 'limit' = ${stats.limit}, responsetime = responsetime || '${Date.now()},' WHERE id = ${interaction.user.id}`);
            ...
            async function matchResult(r) {
                ...
                if (dunLim[0] - stats.limit >= 0 || !myClass) Embed.setFooter({ text: `Balance: ${stats.coins} coins`, iconURL: interaction.user.displayAvatarURL({ dynamic: true }) + "?size=2048" });
                else Embed.setFooter({ text: `${myClass.name} level: ${myStats.clvl} | XP left: ${(myStats.clvl * 50) - (stats.classlevels[myClass.id] - (myStats.clvl * (myStats.clvl - 1) * 25))}`, iconURL: myClass.image });
