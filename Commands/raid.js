...
function getRaidButtonRow(tab, canPlay) {
    const buttons = [
        new ButtonBuilder()
            .setCustomId('play')
...
function rankupOverview(interaction, stats, guild, raid, userItems) {
        ...
        interaction.reply({ embeds: [Embed], components: [getRaidButtonRow(tab, attemptsLeft > 0)], fetchReply: true }).then((msg) => {
            ...
            play.on('collect', () => {
                if (dungeonInProgress.has(stats.id)) {
                    return interaction.channel.send("You already have an fight in progress, please finish it before attempting to start a new one.");
                };
...
module.exports = {
    ...
    execute(interaction) {
        ...
        db.serialize(async () => {
            ...
            const { 0: raid } = await query(`SELECT rowid, * FROM raids WHERE guildid = '${stats.guild}' ORDER BY rowid DESC LIMIT 1`);
            ...
            raid.participation = JSON.parse(raid.participation);
            ...
            async function matchResult() {
                ...
                // Damage dealt & participation
                const damageDealt = (eStats.hp - eStatsC.hp) < 0 ? 0 : (eStats.hp - eStatsC.hp);
                const { 0: damages } = await query(`SELECT enemy_hp, participation FROM raids WHERE rowid = ${raid.rowid}`);
                damages.participation = JSON.parse(damages.participation); // [0: damage, 1: rounds played]
                if (damages.participation[interaction.user.id]) {
                    damages.participation[interaction.user.id][0] += damageDealt;
                    damages.participation[interaction.user.id][1] += 1;
                } else damages.participation[interaction.user.id] = [damageDealt, 1];

                // Update participation
                await query(`UPDATE raids SET enemy_hp = enemy_hp - ${damageDealt}, participation = '${JSON.stringify(damages.participation)}' WHERE rowid = ${raid.rowid}`);
                if ((damages.enemy_hp - damageDealt) < 1) endRaid(raid.rowid); // Finish Raid!



                return new EmbedBuilder()
                    .setColor(currentRaid.accentColor)
            ...
            // Apply passives
            if (skill && myChar.id !== 4767) await skill._passive(myStatsC, eStatsC, buffs, eBuffs, myChar, enemy, matchStats, notice, new EmbedBuilder(), interaction.user, interaction.commandName);
            ...
            if (myStats.weapon !== -1) await items[myStats.weapon]._buff(myStatsC, myStats, eStatsC, buffs, eBuffs, myChar, enemy, matchStats, notice, new EmbedBuilder(), interaction.user);
            if (myStats.shieldid) await items[myStats.shieldid]._buff(myStatsC, myStats, eStatsC, buffs, eBuffs, myChar, enemy, matchStats, notice, new EmbedBuilder(), interaction.user);
            if (myStats.helmet && items?.[myStats.helmet].setname === items?.[myStats.cuirass]?.setname && items?.[myStats.helmet].setname === items?.[myStats.gloves]?.setname && items?.[myStats.helmet].setname === items?.[myStats.boots]?.setname) await items[myStats.boots]._buff(myStatsC, myStats, eStatsC, buffs, eBuffs, myChar, enemy, matchStats, notice, new EmbedBuilder(), interaction.user);
