    ...
    return new Promise((resolve) => {
        ...
        interaction.reply({ embeds: [Embed], components: [selectionRow, getButtonRow()], fetchReply: true }).then((msg) => {
                ...
                interaction.awaitModalSubmit({ filter: (r) => r.customId === ('edit_craze_' + uid), time: 90000 }).then(async (r) => {
                    ...
                    if (set) {
                        ...
                        if (getSet?.name) {
                            ...
                            if (setItems.find((item) => item.type === "helmet")) stats.craze_equipment.helmet = `${setItems.find((item) => item.type === "helmet").id}:706183309943767112`;
                            if (setItems.find((item) => item.type === "cuirass")) stats.craze_equipment.cuirass = `${setItems.find((item) => item.type === "cuirass").id}:706183309943767112`;
                            if (setItems.find((item) => item.type === "gloves")) stats.craze_equipment.gloves = `${setItems.find((item) => item.type === "gloves").id}:706183309943767112`;
                            if (setItems.find((item) => item.type === "boots")) stats.craze_equipment.boots = `${setItems.find((item) => item.type === "boots").id}:706183309943767112`;
                        };
                        if (set === "remove") {
                    ...
                    // Save build
                    await query(`UPDATE users SET craze_equipment = '${JSON.stringify(stats.craze_equipment)}' WHERE id = ${interaction.user.id}`);
            ...
            select.on('collect', r => {
                ...
                let readVal = r.values[0];
                if (readVal >= levelsUnlocked) readVal = 0;

                level = parseInt(readVal);
...
module.exports = {
    name: 'christmas',
    description: 'Christmas Craze (event game mode)',
    execute(interaction) {
        ...
        db.serialize(async () => {
            ...
            // Party chars
            const partyQuery = await query(`SELECT id, craze_equipment FROM users WHERE party = '${stats.party}' AND craze_equipment LIKE '%char%' AND id != ${interaction.user.id}`);
            let partyChars = partyQuery.map((e) => characters[JSON.parse(e.craze_equipment).char]);

            // Enemy Stats
            let enemy = crazeMobs[level];
            const curseRar = enemy.boss ? curses.filter((e) => e.tier) : curses.filter((e) => e.tier === 0);
            const curse = (level in getCrazeMobCurse) ? curses[getCrazeMobCurse[level]] : curseRar[Math.floor(Math.random() * curseRar.length)];
            ...
            async function matchResult(r) {
                ...
                if (r === "l") {
                    ...
                    if (!(level in stats.craze_levels)) {
                        ...
                        await query(`UPDATE users SET craze_levels = '${JSON.stringify(stats.craze_levels)}' WHERE id = ${interaction.user.id}`);
                    };
