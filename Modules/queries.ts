...
export const getPlayerbaseStats = async (): Promise<{ players: number; active: number; daily: number; }> => {
    const [result] = await query(
        `SELECT 
            COUNT(rowid) AS players,
            COUNT(lastonline) FILTER (WHERE lastonline > NOW() - INTERVAL '7 days') AS active,
            COUNT(lastonline) FILTER (WHERE lastonline > NOW() - INTERVAL '1 day') AS daily
        FROM users`
    ) as [{ players: number; active: number; daily: number; }];
    return result;
};
