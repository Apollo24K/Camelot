...
import { getDetailedStats, dealDamage, addHeal, getRefinement, noTimeout, imageChange, procburn } from "./functions";
...
export const abilities: Record<number, Ability> = {
    ...
    "735": {
        ...
        used: 0,
        cost: 65,
        desc: "**Total Usage**: `10`\n**Mana**: `65`\\💧\n**Timeout**: `Yes`\n**Role**: `DPS`\n\nEach use of Yoimiya's normal attack will grant her a 'flame', up to **20**. After collecting three 'flames', her normal attack receives a substantial **22.5%** increase in damage. Additionally, if Yoimiya is wielding a bow as her primary weapon, her normal attacks will additionally deal **12.5%** true damage for **2** rounds, and apply BURNING [ <a:burn:1475075402295803914> ] for **1** round.\n\nHer active ability has her deliver a one-two punch of **80%** physical and magical damage each, before unleashing a festive reprise, dealing **10%** DMG for every flame collected, for a maximum of **100%**. This will also trigger BURNING once if available. The next round after using her active ability, her normal attack will trigger twice.\n\nYoimiya is **not** compatible with other ATK replacing abilities.",
        shortdesc: "**Uses**: `10`\n**Cost**: `65 💧`\n**Timeout**: `Yes`\n**Role**: `DPS (Flame)`\n\n__**Passive**__\n\nATTACK is altered to:\n- Grant **1x** `Flame` (Up to 20)\n- Deals **100%** DMG. This is increased to **122.5%** when she has **3** or more `Flame`\n- When equipped with a bow, after a normal ATK, additionally deals **12.5%** true DMG for **2** rounds, and applies BURNING for **1** round.\n\n__**Active**__ (✨)\n- Deals **80%** ATK + **80%** MD\n- Then deals **10%** DMG for every `Flame` owned (up to 100%)\n- Trigger BURNING once if available.\n- Her normal ATTACK next turn will trigger twice\n\nNote:- This unit is incompatible with other ATK-replacing effects",
        ability: async (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) => {
            // Yoimiya
    ...
    "9004": {
        ...
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            ...
            myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + domainLast, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                ...
                // if (eStats.hp < 0) eStats.hp = 0;
                dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `❄️ **Frozen wounds** shattered and`, { overwriteDamage: shatterdmg, magicDamage: true, dodge: false, ignoreShield: true, canCounter: false });
    ...
    "10528": {
        ...
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            ...
            // Active (I)
            if (this.used === 1) {
                // Lose 50% current HP instantly & 4% max HP every round
                ...
                myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    // Gain 0.75/1.5% ATK for every 1 💧consumed
                    const atkScale = myStats.dalusPrime ? 0.015 : 0.0075;
                    ...
                    const dmgLimit = myStats.dalusPrime ? 0.16 : 0.8;
                    const dmg = (eStats.def + eStats.mr < 100000) ? Math.floor(Math.min(eStats.maxhp * 0.05, myStats.atk * dmgLimit)) : 0;
                    // Overflow?
                    if (eStats.maxhp * 0.05 > myStats.atk * dmgLimit) {
                        addHeal(myStats, eStats, myStats, mybuff, ebuff, matchStats, notice, ``, Math.floor(Math.min(eStats.maxhp * 0.05 - myStats.atk * dmgLimit, myStats.maxhp * (myStats.dalusPrime ? 0.08 : 0.04))), {});
                    };
                ...
                if (!myStats.dalusPrime) {
                    matchStats.interaction.followUp({ content: `🌹 **Dalus** needs to be in his PRIME state to use his active`, ephemeral: true });
                    this.used--;
                    return AbilityResponse.FAILURE;
                };
                // 9 round cd
        ...
        party: async (pStats, myStats, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
            const names = matchStats.partyChars.map((e: IcharInfo) => e.name);
            const dmgScale = names.includes("Kisogi") ? 0.1 : 0.08, critChance = names.includes("Kisogi") ? 1 : Math.random(), dmg = (eStats.def + eStats.mr < 100000) ? Math.floor(myStats.atk * dmgScale) : 0, desc = names.includes("Kisogi") ? `**✨ Kisogi**` : `<:rosie:1408505520641409127> **Rosie**`;
    ...
    "12398": {
        ...
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            ...
            if (this.used === 2) {
                ...
                eStats.mr -= Math.min(Math.floor(eStats.mr * defShred), 660);
                ebuff.mr.push(new buffInfo("+", -Math.min(Math.floor(eStats.mr * defShred), 660), 9999));
                notice.push(`\n<:dream2:1449402981320626186> ${char.name} entered a __Wild Dream__`);
                dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `💦 **Kisogi**`, { atkMultiplier: dmgScale, magicDamage: true, dodge: false });
            };
                ...
                if (!myStats.kisogiPrime) {
                    ...
                    myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 10, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                        if (eStats.recordDamageDealt > myStats.recordDamageDealt) {
                            ...
                            notice.push(`\n<:dream3:1449747554517389413> **Kisogi** dealt more damage than the enemy, and will stay in __??? Dream__ permanently.`);
                        }
                        return AbilityResponse.SUCCESS;
                    }));
                }
    ...
    "13000": {
        ...
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            ...
            // Tomori Kick
            dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `✨ Tomori Kick! She`, { atkMultiplier: 1.2, magicDamage: true, combodmg: true, selfdmg: true, selfheal: true });
    ...
    "14909": {
        ...
        passive: async function (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) {
            ...
            matchStats.on("counter", ({ trigger, caster, target, casterBuff, targetBuff, matchStats, options }: any) => {
                ...
                // Shuhu's Gift
                if (myStats.charge >= 6 && myStats.vengeanceLastRound !== matchStats.round) {
                    myStats.vengeanceLastRound = matchStats.round;
                    myStats.charge -= 6;
                    const bonus = Math.floor(myStats.damageTaken * 0.15);
                    myStats.atk += Math.min(Math.floor(myStats.atk * 0.4), bonus);
                    addHeal(myStats, eStats, myStats, mybuff, ebuff, matchStats, notice, ``, Math.min(Math.floor(myStats.maxhp * 0.4), bonus));
            ...
            myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                if ((myStats.hp / myStats.maxhp < 0.5 || this.bladeUsedActive) && myStats.damageReduction < 0.28) myStats.damageReduction = 0.28;
                if (myStats.charge >= 6 && myStats.vengeanceLastRound !== matchStats.round) {
                    myStats.vengeanceLastRound = matchStats.round;
                    myStats.charge -= 6;
                    const bonus = Math.floor(myStats.damageTaken * 0.15);
                ...
                if (matchStats.round % 4 === 0) {
                    ...
                    if (!names.includes("Kafka")) {
                        ...
                        if ((matchStats.round >= 40 && myStats.name !== "Kafka") || myStats.name == "Dan Heng") {
                            dealDamage(myStats, eStats, mybuff, ebuff, matchStats, notice, `🩸 **${pStats.name}** attacked **${myStats.name}**! He`, { atkMultiplier: 0.15 });
                        };
                    }
    ...
    "16195": {
        ...
        passive: async function (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) {
            ...
            myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                ...
                // Blood Debt Directive effects (if any)
            ...
            myStats.replaceButton.cskill = {
                ...
                "run": async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    ...
                    if (cd > 0) {
                        ...
                        const consumableBOL = Math.floor(Math.min(myStats.maxhp * 0.5, myStats.bondOfLife));
                        ...
                        dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `❌ **${char.name}**`, { overwriteDamage: Math.floor(myStats.bondOfLife), magicDamage: true, dodge: false });
            ...
            myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                ...
                if (myStats.creampuff > 0) {
                    ...
                    if (myStats.creampuff > 0) {
                        ...
                        notice.push(`\n<a:creampuff:1478055519938805911> **${char.name}** is out of creampuffs and exited Guard Stance.`);
                    };
                };
                return AbilityResponse.SUCCESS;
            }, 9999));
                ...
                callback: ({ trigger, caster, target, casterBuff, targetBuff, matchStats, options }) => {
                    if (caster === myStats) {
                        ...
                        if (myStats.defUsed === 5) {
                            ...
                            notice.push(`\n<a:creampuff:1478055519938805911> **${char.name}** lost all creampuffs and converted block rate to **${Math.floor(ccGain * 100)}%** counter chance.`);
                        };
                        return true;
                    };
                },
        ...
        party: async (pStats, myStats, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
            ...
            myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                ...
                if (0.75 > Math.random() && myStats.creampuff < 10) {
                    myStats.creampuff++;
                    notice.push(`\n<a:creampuff:1478055519938805911> **${char.name}** has received a creampuff and have ${myStats.creampuff} creampuffs.`);
                };
    ...
    "19050": {
        ...
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            ...
            if (this.weaponType === "sword") {
                ...
                dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `✨ **${char.name}** used Musou no Hitotachi! She`, { atkMultiplier: 1.2, magicDamage: true, combodmg: true, isLightning: true, selfdmg: true, selfheal: true, ignoreShield: true });
                ...
                myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    if (myStats.hp / myStats.maxhp > 0.5) {
                        ...
                        if (!myStats.raidenHpDownRound) {
                            dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `✨ **${char.name}** fell below 50% HP! She`, { atkMultiplier: 1.8, isLightning: true, magicDamage: true, combodmg: true, selfdmg: true, selfheal: true, ignoreShield: true });
        ...
        party: async function (pStats, myStats, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) {
            if (pStats.weapon !== -1 && items[pStats.weapon].type === "sword") {
                const name = pStats.name;
                dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `✨ **${name}**`, { atkMultiplier: 0.14, isLightning: true, ignoreShield: true, magicDamage: true });
                myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `✨ **${name}**`, { atkMultiplier: 0.14, isLightning: true, ignoreShield: true, magicDamage: true });
    ...
    "23185": {
        ...
        pause: 0,
        selfhealidx: 0,
        desc: "**Total Usage**: `1 (CD: 10) + 1`\n**Cost**: `20 💧+ 0💧`\n**Timeout**: `No/Yes`\n\n-# Being YorHa androids, 2B and 9S complement each other in combat as a duo, cleaving through machines, fighting for a future where humanity on the moon could regain control over the world.\n\nSpecifically programmed to operate indefinitely, androids are especially resistant to damage, having **+15%** max HP. Moreover, they may upload their data to their headquarters, allowing them a **100%** chance to revive with **50%** HP upon death.\n\nThey are equipped with their pods, allowing them to equip up to **2** programmes which take effect in battles via `/item equip item:prog <ID>`. To view programmes, do `item equip item:prog info`. To reset programmes, do `/item equip item:prog remove` instead.\n\nIn combat, using ATTACK allows 2B to spring into the air, losing **7%** current HP (doesn't lose after reviving) before dealing **80%** DMG, before slamming them in a strike, dealing an additional instance of **10%** DMG, further increased by **1%** for every **1%** HP missing from the enemy (Up to 40% in total).\n\nEvery critical hit allows 9S to analyze the foe, granting **1x** [ɪɴꜱɪɢʜᴛ]. At the start of a round, when 9S is not HACKING but owns **8x** or more [ɪɴꜱɪɢʜᴛ], 9S consumes **8x** [ɪɴꜱɪɢʜᴛ] and begins HACKING for **6** rounds.\n\nHACKING: 9S initiates hacking on the enemy while 2B supports by distracting the foe. Critical hits deal **+1%** DMG for every **1** percentage point of remaining HP% difference between you and the enemy, up to **+33%**. Moreover, non-critical hits cause him to lose **4%** current HP.\n\nThe duo’s active is split into 2 parts. The First use allows 2B & 9S to enter their respective flight unit, gaining **+15%** ATK & DEF for **15** rounds.\n\n The Second use causes self-destruction, colliding their black boxes, dealing **100%** max HP as critical absolute damage to the enemy before *dying*.\n\nIn a party, the duo shares their pod passives with the entire team.",
        shortdesc: "**Uses**: `1+1`\n**Cooldown**: `10 rounds`\n**Cost**: `20 💧 // 0 💧`\n**Timeout**: `No / Yes`\n**Role**: `DPS (Sacrificial, Critical, Revival)`\n__**Passive**__\n- Upon death, has a **100%** chance of reviving with **50%** HP.\n- **+15%** max HP\n- They may equip **2** programmes on pod for battle effects. To view available options, do `/item equip item: prog info`. To equip, do `/item equip item:prog <ID>`. To reset, do `/item equip item:prog remove`.\n\nATTACK is altered:\n> - Loses **7%** current HP (doesn't lose after reviving)\n> - Deals **80%** DMG, before dealing another instance of **10%**, further increased by **1%** for every **1%** HP missing from the enemy (Up to 40% in total).\n\n- Every critical hit grants **1x** [ɪɴꜱɪɢʜᴛ].\n\n__Core Mechanic__: HACKING\n- At the start of the round, when owning **8x** [ɪɴꜱɪɢʜᴛ] while not *HACKING*: 9S consumes **8x** [ɪɴꜱɪɢʜᴛ] and begins *HACKING* for **6** rounds.\n\nDuring *HACKING*:\n- Critical hits deal **+1%** DMG for every **1** percentage point of remaining HP% difference between you and the enemy, up to +33%.\n- Non-critical hits cause him to lose **7%** current HP\n\n__**Active**__:\n__First use__: Gain **15%** ATK & DEF for **15** rounds\n\n__Second use__: Deals **100%** max HP as a critical hit to the enemy as absolute damage before *dying*.\n\n__**Party**__:\n- Shares equipped pod passive with entire party",
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            // 2B & 9S EX | 2B&9S EX

            // Enter respective flight unit for 15 rounds
            ...
            if (this.used === 1 && myStats.sm < 20) {
                ...
                notice.push(`\n✨ **${char.name}** entered their flight units for **15** rounds.`);
                myStats.atk += Math.floor(myStats.atk * 0.15);
                myStats.def += Math.floor(myStats.def * 0.15);
                mybuff.atk.push(new buffInfo("*", 1.15, 15));
                mybuff.def.push(new buffInfo("*", 1.15, 15));
                /* SIMPLIFICATION
        ...
        passive: async function (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) {
            ...
            // +15% max HP ; 100% chance to revive with 50% HP
            myStats.rev ??= 0;
            myStats.revhp ??= 0.5;
            myStats.maxRevivals ??= 0;
            ...
            myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                ...
                */
                ...
                if (myStats.insight >= 8 && !myStats.hacking) {
                    notice.push(`\n<:9SHack:1373261619924172940> 9S initiated hacking for **6** rounds`);
                    myStats.insight -= 8;
                    myStats.hacking = true;

                    let bonus = Math.min(0.33, Math.abs((myStats.hp / myStats.maxhp) - (eStats.hp / eStats.maxhp)));
            ...
            matchStats.on("noncrit", ({ trigger, caster, target, casterBuff, targetBuff, matchStats, options }) => {
                if (caster === myStats && myStats.hacking === true) { // Lose 4% current HP when non-crit during hacking
                    myStats.hp -= Math.floor(myStats.hp * 0.04);
                };
            });
    ...
    //         this.passivePause = 0;
    //         const dodgeEmoji = myStats.equippedSkin === 112 ? "<:dodgeCosmic:1446888994154610840>" : myStats.equippedSkin === 127 ? "<:dodgeMonster:1446889038098468938>" : "💨";
    ...
    "24185": {
        ...
        pause: -15,
        nsbTick: 0,
        desc: "**Total Usage**: `Unlimited`\n**Cost**: `75 💧` / `All fighting spirit` (<a:fighting_spirit:1519896326882594957>)\n**Timeout**: `Yes / Yes`\n**Cooldown**: `15 rounds (None if in NBS)` / `None`\n**Tags**: `DPS (DoT, Burst)`\n\n## NBS - Nightsoul Blessing state:\nWhen Mavuika has at least **5** Nightsoul points, enters this state. This consumes **5** Nightsoul points each round, granting her **0.5%** crit. rate and crit. damage for each nightsoul point consumed (capped at 25% each). This state ends once her Nightsoul points are exhausted, also expiring the buffs thus gained.\n\n## Active (I) : The Named Moment\nCalling upon her authority over conflict, Mavuika summons the All-Fire Armaments passed down through the line of human Archons (deals **25%** burn damage every **3** rounds, effect expires when exiting NBS), before dealing **100%** burn DMG. After which, her Nightsoul points are restored to **40**.\n\n## Active (II) : Hour of Burning Skies\nMavuika's burst is not dependent on mana, but Fighting Spirit (<a:fighting_spirit:1519896326882594957>). When Mavuika is not in NBS and has ≥ **50** <a:fighting_spirit:1519896326882594957>, her active is altered to consume all the fighting spirit to unleash her Elemental Burst: Mavuika gains **20** Nightsoul points and enters NBS. Riding her Flamestrider high in the air, she uses a powerful Sunfell Slice, dealing **180%** burn DMG, for each :fighting_spirit: consumed in excess of **50**, increases burst DMG by **2%**.\n\n## Fighting Spirit (<a:fighting_spirit:1519896326882594957>):\nMavuika can obtain <a:fighting_spirit:1519896326882594957> via the following methods (max 100): Nightsoul points consumed are converted to <a:fighting_spirit:1519896326882594957>.\nWhen hitting opponents, Mavuika gains **1** <a:fighting_spirit:1519896326882594957>.\n\nIn a party, she provides a **12%** DEF/MR reduction (Up to 2x damage), and deals **25%** burn DMG every **3rd** round.",
        shortdesc: "**Uses**: `Unlimited`\n**Cost**: `75 💧 / All fighting spirit` (<a:fighting_spirit:1519896326882594957>)\n**Timeout**: `Yes / Yes` \n**Cooldown**: `15 rounds (None if in NBS) / None`\n**Role**: `DPS (DoT, Burst)`\n\n**__Active (I)__** : The Named Moment\n- Deals **25%** burn damage every **3** rounds, effect expires when exiting NBS), before dealing **100%** burn DMG. After which her Nightsoul points are restored to **40**.\n\n__**Core Mechanic (I)**__\n`NBS`:\n- When Mavuika has at least **5** Nightsoul points, enters this state.\n- Consumes **5** Nightsoul points each round, granting her **0.5%** crit. rate and crit. damage for each nightsoul point consumed (capped at 25% each).\n- NBS ends once her Nightsoul points are exhausted, also expiring the buffs thus gained.\n\n**__Active (II)__** : Hour of Burning Skies\n- Mavuika's burst is not dependent on mana, but Fighting Spirit (<a:fighting_spirit:1519896326882594957>).\n- When Mavuika is not in NBS and has ≥ **50** <a:fighting_spirit:1519896326882594957>, her active is altered to consume all <a:fighting_spirit:1519896326882594957> to unleash her Elemental Burst:\n- Mavuika gains **20** Nightsoul points and enters NBS. Riding her Flamestrider high in the air, she uses a powerful Sunfell Slice, dealing **180%** burn DMG, for each <a:fighting_spirit:1519896326882594957> consumed in excess of 50, increases burst DMG by **2%**.\n\n__**Core Mechanic (II)**__\n`Fighting Spirit` (<a:fighting_spirit:1519896326882594957>):\n> Mavuika can obtain <a:fighting_spirit:1519896326882594957> via the following methods (max 100):\n- Nightsoul points consumed are converted to <a:fighting_spirit:1519896326882594957>.\n- When hitting opponents, Mavuika gains **1** <a:fighting_spirit:1519896326882594957>.\n\n__**Party Ability**__:\n- Provides a **12%** DEF/MR reduction (Up to 2x damage), and deals **25%** burn DMG every **3rd** round.",
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            // Mavuika
            if (myStats.fightspirit < 50 || myStats.nbs > 0) {// Active (I)
                ...
                dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `🔥 **${char.name}**`, { atkMultiplier: 1, isBurn: true });
            } else {
                ...
                if (matchStats.round % 3 === 0) dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `🔥 Divine Name Unleashed... **${char.name}**`, { atkMultiplier: 0.25, isBurn: true });

                const flair = ["Wrath from the skies!", "Pyroclastic surge!", "Light of the blazing sun!"];
                dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `🔥 ${flair[Math.round(Math.random() * 2)]} **${char.name}**`, { atkMultiplier: 1.8 + 0.02 * (myStats.fightspirit - 50), isBurn: true });
        ...
        passive: async function (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) {
            ...
            switch (matchStats.interaction.commandName) {
                case "dungeon": notice.push(`\n<a:fighting_spirit:1519896326882594957> Dungeons? Adventuring is right up my alley.`); break;
                case "stampede": notice.push(`\n<a:fighting_spirit:1519896326882594957> I'm going to take it easy and wrap up some work. Oh, don't worry — I see keeping busy as a great way to relax.`); break;
                case "arena": notice.push(`\n<a:fighting_spirit:1519896326882594957> If you want to reach your full potential, you need to be willing to become the weakest link.`); break;
                default: notice.push(`\n<a:fighting_spirit:1519896326882594957> Ready to ride, any time, any place.`); break;
            };
        ...
        party: async (pStats, myStats, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
            ...
            myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                if (matchStats.round % 3 === 0) {
                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `🔥 **${pStats.name}**`, { atkMultiplier: 0.25, isBurn: true });
                };
    ...
    "24798": {
        ...
        cost: 0,
        pause: -11,
        desc: "**Total Usage**: `Unlimited (CD: 10)`\n**Cost**: `25% current HP`\n**Timeout**: `false`\n**Role**: `DPS (Non-critical)`\n\n-# Built out of high-energy pure crystals, Shorekeeper acts as a vessel where data filled with grief, chaos and decay flow through her for analysis. She repairs her scars with new crystals, as she witnesses the tragedies of humanity.\n\nHer ATTACK is altered to __Origin Calculus__, interpreting the reverberations. She deals **80%** DMG. For every `🦋` summoned, this hit’s damage scaling is increased by **6%**. If the hit is non-critical, gains **3x** `💠`. When she has **5x** or more `💠`, she immediately follows up with __Astral Chord__: For every **5** existing `💠`, transforms them into **1x** `🦋` (Up to 10). The enemy will also have **-2%** DEF & MR for **3** rounds for every `🦋`. (Max 1.5x DMG, each shred calculated independently)\n\nConsuming **25%** of her current HP, she casts her active, __End Loop__, where she summons a domain of Stellarealm for **10** rounds. During this period, She restores **5%** lost HP every round and has **+3%** ATK & MD for every **1x** `🦋` owned. Moreover, attacks will *benefit from the critical damage scaling* even if they don’t land a critical strike\n\nRight before exiting the domain, she increases critical DMG by **30%** for **1** round, before giving a final ordination.\n- [Default] : Deals **70%** DMG\n- [5+ `🦋`] : Deals **210%** DMG + restores **15%** max HP\n- [10 `🦋`] : Deals **300%** DMG + restores **30%** max HP\n\nIn a party, whenever the ally lands a non-critical hit, they gain **1.5%** dodge rate, up to **15%**. Moreover, allies evade the first **2** lethal attacks and restore **15%** max HP right afterwards.",
        shortdesc: "**Uses**: `Unlimited`\n**Cooldown**: `10 rounds`\n**Cost**: `25% current HP`\n**Timeout**: `No`\n**Role**: `DPS (Non-critical)`\n\n__**Passive**__\n- ATTACK is altered:\n- Deals **80%** DMG\n- For each `🦋` summoned, DMG scaling is increased by **6%**\n- If this hit is non-critical, grant **3x** additional `💠`\n- If amount of `💠` reaches **5** or more, follows up with “Astral Chord”\n\n“Astral Chord”:\n- For every **5** existing `💠`, transforms them to **1x** `🦋` (Up to 10)\n- Reduces enemy’s DEF/MR by **2%** for **3** rounds for every **1** `🦋` owned (Max 1.5x DMG, each shred calculated independently)\n\n__**Active**__ (✨)\nCreates a domain of Stellarealm for **10** rounds, during this period:\n- Restores **5%** missing HP every round\n- Boosts ATK & MD by **3%** for every **1x** `🦋` owned\n- All attacks **will benefit from the critical damage scaling** even if they don’t land a critical strike\n\nRight before exiting the domain:\n- Increases critical DMG by **30%** for **1** round\n- Grants additional effects based off `🦋` owned\n> - [Default] : Deals **70%** DMG\n> - [5+ `🦋`] : Deals **210%** DMG + restores **15%** max HP\n> - [10 `🦋`] : Deals **300%** DMG + restores **30%** max HP\n- The hit aforementioned will not reset combos\n\n__**Party**__ (👥):\nAfter the ally lands a non-critical strike: Increases ally’s dodge rate by **1.5%** (Up to 15%)\n- Allies evade the first **2** lethal attacks and restores **15%** max HP (Up to 2 times)",
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            // Shorekeeper EX
            noTimeout(matchStats, myStats);
    ...
    "25828": {
        ...
        cost: 80,
        pause: -7,
        desc: "**Uses**: `Unlimited (CD: 6)`\n**Cost**: `80 💧`\n**Timeout**: `False`\n**Role**: `DPS (Stability, Weakpoint)`\n\n-# Yor Forger may seem like an ordinary office clerk on the surface, but is secretly a Garden assistant, known for her great physical power through punches and kicks.\n\nAt the onset of the fight, she lands a surprise attack, dealing **150%** critical true damage to the enemy. Moreover, she interacts with the <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> [ [ <a:stability:1451561886339436675> ] ] effect. The enemy enters battles with **100%** <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>. Every critical strike landed on the enemy reduces their <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> by **2%**. When it reaches **0%**, triggers `WEAKPOINT`, stunning the enemy and lowering their DEF/MR by **50%** (Up to 2.5x damage) the next **2** rounds. After **4** rounds, <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> resets to **100%**.\n\nWith great intuition, Yor can often execute opponents in rapid succession, through multiple consecutive actions. Her active (:sparkles:) allows her to make haste for **3** rounds. During this period, every time she lands a critical hit, she can make another action (counted as no timeout, up to **2** times).\n\nBy the end of the effect, for every time she gained another action, she has a **20%** chance to disarm the enemy, lowering their ATK & MD by **15%**, while doubling the <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> loss from critical strikes for **3** rounds, allowing her to quickly break the opponent down.\n\nIn a party, Yor Forger applies her great insight over the battlefield to allies. The <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> and `WEAKPOINT` mechanic is applied universally to all of ally's battles.",
        shortdesc: "< «•   Passives   •» >\n- When starting combat: Deals **150%** critical true Damage.\n\n< «•  __Core Mechanic__   •» >\n> *Yor focuses on accumulating critical strikes to wear down the enemy's defense.* [ <a:stability:1451561886339436675> ]\n- The enemy enters battles with **100%** <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>. Every critical strike landed on the enemy reduces their <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> by **2%**.\n- When <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> reaches **0%**, triggers `WEAKPOINT`, stunning the enemy and lowering their DEF/MR by **50%** (Up to 2.5x damage) the next **2** rounds. After **4** rounds, <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> resets to **100%**.\n\n< «• __Active (✨)__  •» >\nUses: `Unlimited (CD: 6)`\nCost: `80 💧`\nTimeout: `False`\n> *Yor makes haste, making multiple actions every round*\n\nFor **3** rounds:\n- Every time she lands a critical hit: she gains another action (counted as no timeout, up to **2** additional actions)\n- By the end of the effect, for every time she gained another action, she has a **20%** chance to decrease their ATK/MD by **15%** and doubling the <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> lost from critical strikes for **3** rounds\n\n< «• __Party (👥)__  •» >\n- Applies the <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> and `WEAKPOINT` mechanic to all ally's battles",
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            // Yor Forger EX
            noTimeout(matchStats, myStats);
            ...
            myStats.yorUsedActive = true;

            // Exit HASTE
            myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + domainLast, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                if (0.2 * myStats.yorExtraTotal > Math.random()) { // Disarmament
                    eStats.atk *= 0.85;
                    eStats.md *= 0.85;
                    ebuff.atk.push(new buffInfo("*", 0.85, 2));
                    ebuff.md.push(new buffInfo("*", 0.85, 2));
                    myStats.stabilityShred = 2;
                    notice.push(`\n💠**${char.name}** disarmed the enemy and lowered their ATK & MD by **15%**`);

                    myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 3, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                        myStats.stabilityShred = 1;

                        return AbilityResponse.SUCCESS;
                    }));
                };

                myStats.yorUsedActive = false;

                return AbilityResponse.SUCCESS;
            }));

            notice.push(`\n✨ **${char.name}** gained __HASTE__ for ${domainLast} rounds!`);
        ...
        passive: async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
            ...
            myStats.stabilityShred = 1;
            myStats.yorUsedActive = false;
            myStats.yorExtraThisRound = 0;
            myStats.yorExtraTotal = 0;
            ...
            matchStats.on("crit", ({ trigger, caster, target, casterBuff, targetBuff, matchStats, options }: any) => {
                if (caster === myStats && target === eStats) {
                    eStats.stability -= 2 * myStats.stabilityShred;
                    if (eStats.stability <= 0 && !myStats.ignoreSTABILITY) {// Weakpoint
                        ebuff.def.push(new buffInfo("+", -Math.min(eStats.def * 0.5, 872), 2));
                        ebuff.mr.push(new buffInfo("+", -Math.min(eStats.mr * 0.5, 872), 2));
                        myStats.ignoreSTABILITY = true;

                        // Stun
                        eStats.timeFrozen = true;
                        let lastfreezemsg = eStats.frozenMessage ? eStats.frozenMessage : "was stunned 💠";
                        ...
                        notice.push(`\n💠 **${char.name}** broke the enemy's stability and reduced their DEF/MR by **50%** the next **2** rounds.`);

                        // Reset stability back to 100%
                        myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 4, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                            eStats.stability = 100;
                            myStats.ignoreSTABILITY = false;
                    ...
                    // If less than 3 actions made this round, timeout false the action
                    if (myStats.yorUsedActive && myStats.yorExtraThisRound < 2) {
                        noTimeout(matchStats, myStats);
                        myStats.yorExtraThisRound++;
                        myStats.yorExtraTotal++;
                    };
                };
            });
            ...
            // At the start of the fight, deal 150% critical true damage
            dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `✨ **${char.name}**`, { atkMultiplier: 1.5, ignoreShield: true, critChance: 1 });

            return AbilityResponse.SUCCESS;
        },
        party: async (pStats, myStats, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {

            // Share stability mechanic
            eStats.stability = 100;
            myStats.stabilityShred = 1;
            myStats.ignoreSTABILITY = false;
            ...
            matchStats.on("crit", ({ trigger, caster, target, casterBuff, targetBuff, matchStats, options }: any) => {
                if (caster === myStats && target === eStats) {
                    ...
                    if (eStats.stability <= 0 && !myStats.ignoreSTABILITY) {// Weakpoint
                        ...
                        notice.push(`\n💠 **${pStats.name}** broke the enemy's stability and reduced their DEF/MR by **50%** the next **2** rounds.`);
    ...
    "6309": {
        ...
        used: 0,
        cost: 0,
        desc: "**Role**: `DPS/Support (Stability, Weakpoint)`\n\n-# Loid forger is an undercover spy, adept at makeshifting and completing whatever missions given at quick speed.\n\nHe interacts with the stability [ <a:stability:1451561886339436675> ] effect. The enemy enters battles with **100%** <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>. Every critical strike landed on the enemy reduces their <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> by **2%**. When <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> reaches **0%**, triggers `WEAKPOINT`, stunning the enemy and lowering their DEF/MR by **50%** (Up to 2.5x damage) the next **2** rounds. After **4** rounds, <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> resets to **100%**.\n\nHis own ATTACK is altered depending on enemy's <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>. If the enemy has **50%+** stability, it deals **80%** damage and gives himself **24%** incoming damage mitigation. Else, he uses [Twilight].\n\n[Twilight] : Increases own ATK by **1%** for every **2%** missing stability from the enemy, up to +28%, for **1** round. Then he deals **140%** undodgeable damage. If the enemy is in `WEAKPOINT`, instead deals **170%** undodgeable damage.\n\nIn a party, loid analyzes all personnel for outcome maximization. If the <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> mechanic is active, the enemy will only have **1%** <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> upon entering battle. Moreover, after the reset of <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>, the enemy immediately loses **33%** <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>. Else, the ally will have **+12%** ATK simply.",
        shortdesc: "**Tags**: `DPS (Stability, Nuke)`\n\n< «• Core Mechanic •» >\n> Loid focuses on accumulating critical strikes to wear down the enemy's defense. [ <a:stability:1451561886339436675> ]\n- The enemy enters battles with **100%** <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>. Every critical strike landed on the enemy reduces their <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> by **2%**.\n- When <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> reaches **0%**, triggers `WEAKPOINT`, stunning the enemy and lowering their DEF/MR by **50%** (Up to 2.5x damage) the next **2** rounds.\n- After **4** rounds, <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> resets to **100%**.\n\n< «• Passives •» >\n> Loid dons a separate identity for every mission, only showing his true colors when necessary\n- Own ATTACK ( :crossed_swords: ) is altered depending on the enemy's stability.\n- If the enemy has **50%+** stability: Deals **80%** damage and gain **24%** incoming damage mitigation\n- Else, own ATK is instead altered to [Twilight]\n[Twilight] :\n- The lower the enemy's <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>, the higher his ATK (**2%** = **1%** ATK, up to +28% ATK). This ATK boost lasts for that round only.\n- Deals **140%** undodgeable damage. If the enemy is in `WEAKPOINT`, instead deals **170%** undodgeable damage\n\n< «• Party (:busts_in_silhouette:) •» >\n> Loid requires time to analyze all personnel for outcome maximization.\n- If the <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> mechanic is active, the enemy will only have **1%** <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> upon entering battle. Moreover, after the reset of <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>, the enemy immediately loses **33%** <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>.\n- Else, the ally will have **+12%** ATK.",
        passive: async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
            // Loid Forger
            eStats.stability = 100;
            myStats.stabilityShred = 1;
                ...
                if (caster === myStats && target === eStats) {
                    ...
                    if (eStats.stability <= 0 && !myStats.ignoreSTABILITY) {// Weakpoint
                        ...
                        myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 4, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                            eStats.stability = 100;
                            myStats.ignoreSTABILITY = false;
                            notice.push(`\n💠 **${enemy.name}** regained **100%** <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>.`);
                            matchStats.trigger("stabilRefresh", myStats, eStats, mybuff, ebuff);
            ...
            myStats.replaceButton.atk = {
                "run": async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    if (eStats.stability > 50) {
                        // Normal ATK - 80% damage & gain 24% damage mitigation
                        dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `<:loidATK1:1446881047206957076> Respectfully... **${char.name}**`, { atkMultiplier: 0.8 });
                        myStats.putDamageOnHold = 0.24;
                    } else {
                        // Operation STRIX - Twilight
                        myStats.atk += Math.floor(myStats.atk * 0.01 * Math.min(Math.floor((100 - eStats.stability) / 2), 28));
                        if (eStats.stability > 0) {
                            // Enhanced ATK (I)
                            dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `<:loidATK2:1446881024020840592> Anya... **${char.name}**`, { atkMultiplier: 1.4, dodge: 0 });
                        } else {
                            // Enhanced ATK (II)
                            dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `<:loidATK3:1446881000180420799> I beg your pardon. **${char.name}**`, { atkMultiplier: 1.7, dodge: 0 });
                        };
                    };
                    return AbilityResponse.SUCCESS;
                },
            ...
            // Reset damage mitigation when enemy has positive 
            myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                myStats.putDamageOnHold = 0;
        ...
        party: async (pStats, myStats, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
            ...
            if (eStats.stability) {
                if (eStats.stability > 1) {
                    eStats.stability = 1;
                    notice.push(`\n<a:stability:1451561886339436675> **${pStats.name}** lowered the enemy's <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866> to **1%**`);
                };
                matchStats.on("stabilRefresh", ({ trigger, caster, target, casterBuff, targetBuff, matchStats, options }) => {
                    if (target == eStats) {
                        eStats.stability -= 33;
                        notice.push(`\n<a:stability:1451561886339436675> **${enemy.name}** lost **33%** <:stab1:1464513839323414549><:stab2:1464513732926509161><:stab3:1464513789935353866>`);
                    };
                });
            } else {
                myStats.atk += Math.floor(myStats.atk * 0.12);
    ...
    "25829": {
        ...
        used: 0,
        cost: 60,
        desc: "**Uses**: `1`\n**Cost**: `60 💧`\n**Timeout**: `True`\n**Role**: `DPS (Progressive, Nuke, Freeze)`\n\n-# Aneira, the queen of frost, commands ice to move fluidly with her hands, able to pierce through armor and glide through flesh, staining the ice with red.\n\nAll of her + her allies' attacks, if they don't carry an element (e.g. Fire / Lightning), are identified as <a:frost1:1504497507185725473><a:frost2:1504497408233705512>, and apply **1x** Glacio Chafe (considered as Frost [`❄️`]).\n- `❄️` : when attacked with a Frost hit, deals **4%** damage for every stack, up to **40%** (once per round). She passively gains **1** icicle ( <:icicle:1446880458498375701> ) every round, up to **40**. Using her active (:sparkles:), she no longer gains <:icicle:1446880458498375701>, permanently. Her ATK is altered to consume **1** <:icicle:1446880458498375701> to deal **60%** damage. If <:icicle:1446880458498375701>  >  20, the damage scaling is increased to **150%**. Her class skill is also altered, consuming **5** to deal **200%** damage. If <:icicle:1446880458498375701> ≤ 20, the skill hits twice. \n\nWhen <:icicle:1446880458498375701> falls to **0** at the start of a round, she unveils her mask, dealing **60%**, **80%** and alas **220%** damage, before freezing the enemy for **3** rounds, ensuring they remain in place for her brutal wrath of chill to any that oppose her.\n\nIn a party, she extends her manipulation of ice, allowing frost hits to deal **20%** more damage, and apply **1** additional Frost.",
        shortdesc: "**Uses**: `1`\n**Cost**: `60 💧`\n**Timeout**: `True`\n\n**__Passive__**: All of her + her allies' attacks, if they don't carry an element (e.g. Fire / Lightning), are identified as <a:frost1:1504497507185725473><a:frost2:1504497408233705512>, and apply **1x** Glacio Chafe (considered as Frost [`❄️`]).\n- `❄️` : when attacked with a Frost hit, deals **4%** damage for every stack, up to **40%** (once per round). Gain **1** `Icicle` (<:icicle:1446880458498375701>) every round (Up to 40).\n\n**__Active (✨)__ - <Cross not my path>**\n> ?? enters a special state, utilizing her Icicles.\n- Stop gaining <:icicle:1446880458498375701> permanently.\n- ATK (:crossed_swords:) is altered: Consume **1** <:icicle:1446880458498375701> to deal **60%** damage. If <:icicle:1446880458498375701> > 20, the damage scaling is increased to **150%**.\n- Class Skill (:fleur_de_lis:) is altered: Consume **5** <:icicle:1446880458498375701> to deal **200%** damage. If <:icicle:1446880458498375701> ≤ 20, the skill hits twice.\n- When <:icicle:1446880458498375701> falls to **0** at the start of a round, she unveils her mask, hitting **3** times (**60+80+220%** damage), then freezes the enemy for **3** rounds.\n\n**__Party (👥)__ \n- Frost hits deal **20%** more damage and apply **1** more Frost.",
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            // Aneira EX
            myStats.aneiraUsedActive = true;
            myStats.element = 1; // Frost damage

            myStats.replaceButton.atk = {
                "emoji": "<:AneiraATK:1448861017659539630>",
                "run": async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    if (myStats.icicle < 1) {
                        noTimeout(matchStats, myStats);
                        matchStats.interaction.followUp({ content: `**${char.name}** does not have an Icicle (${myStats.icicle}/1 <:icicle:1446880458498375701>)`, ephemeral: true });
                        return AbilityResponse.FAILURE;
                    }
                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `<:AneiraATK:1448861017659539630> **${char.name}**`, { atkMultiplier: myStats.icicle > 20 ? 0.6 : 1.5 });
                    myStats.icicle--;
                    if (myStats.icicle === 20) notice.push(`\n**${char.name}** has **20** <:icicle:1446880458498375701> left.`);
                    return AbilityResponse.SUCCESS;
                },
            };

            myStats.replaceButton.cskill = {
                "emoji": "<:icicle:1446880458498375701>",
                "run": async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    if (myStats.icicle < 5) {
                        noTimeout(matchStats, myStats);
                        matchStats.interaction.followUp({ content: `**${char.name}** does not have sufficient Icicles (${myStats.icicle}/5 <:icicle:1446880458498375701>)`, ephemeral: true });
                        return AbilityResponse.FAILURE;
                    }
                    if (myStats.icicle <= 20) dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `⚜️ Quiet... **${char.name}**`, { atkMultiplier: 2 });
                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `⚜️ Seal... **${char.name}**`, { atkMultiplier: 2 });
                    myStats.icicle -= 5;
                    return AbilityResponse.SUCCESS;
                },
            };

            // If Icicle reaches 0 -> Unveil
            myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                if (myStats.icicle === 0) {
                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `<:icicle:1446880458498375701> Foul. **${char.name}**`, { atkMultiplier: 0.6 });
                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `<:icicle:1446880458498375701> Pest. **${char.name}**`, { atkMultiplier: 0.8 });
                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `<:icicle:1446880458498375701> BEGONE! **${char.name}**`, { atkMultiplier: 2.2 });
                    eStats.timeFrozen = true;
                    ...
                    myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 3, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                        eStats.timeFrozen = false;
                        eStats.frozenMessage = lastfrozenmsg;
                        return AbilityResponse.SUCCESS;
                    }));
                };
            ...
            notice.push(`\n✨ **${char.name}** casted her technique -- [Cross not my path].`);
        ...
        party: async (pStats, myStats, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
            // Frost hits deal 20% more damage, and apply 1 extra Frost
            myStats.frostbonus ??= 0;
            myStats.frostbonus += 0.2;
            myStats.extraFrost ??= 0;
            myStats.extraFrost += 1;
    ...
    "25978": {
        ...
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            ...
            if (myStats.ce < 150 && !myStats.rika) {
                noTimeout(matchStats, myStats);
                matchStats.interaction.followUp({ content: `**${char.name}** does not have sufficient Cursed Energy (**${myStats.ce}**/150 <:ce:1466817050860191817>)`, ephemeral: true });
                this.used--;
                return AbilityResponse.FAILURE;
            };

            if (!myStats.rika) {// Active (I)
                if (this.pause > matchStats.round) {
                    noTimeout(matchStats, myStats);
                    matchStats.interaction.followUp({ content: `**${char.name}** needs to rest ${this.pause - matchStats.round} more ${this.pause - matchStats.round === 1 ? "round" : "rounds"}`, ephemeral: true });
                    this.used--;
                ...
                myStats.ignoreRestrain = false;

                // Apply Vulnerability, Freeze Time
                eStats.timeFrozen = true;
                eStats.frozenMessage = "is immobilized by Rika";
                ...
                myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    ...
                    if (Math.random() < (0.4 + 0.1 * restrainedRounds) || matchStats.round === this.pause + 5) {
                        ...
                        eStats.timeFrozen = false;
                        dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `✨ **${char.name}** landed a strike in fury! He`, { atkMultiplier: 0.3 * (5 - restrainedRounds), ignoreShield: true });
                        myStats.ignoreRestrain = true;
                    };
                    return AbilityResponse.SUCCESS;
                }, 5));
                ...
                myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 7, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    myStats.rika = false;
                    notice.push(`\n✨ **${char.name}** retreated his summon`);
                    return AbilityResponse.SUCCESS;
                }));
                ...
                // Love Beam (Active II)
                if (myStats.activeIIused === 3) {
                    noTimeout(matchStats, myStats);
                    matchStats.interaction.followUp({ content: `**${char.name}** can only cast LOVE BEAM **3** times in battle`, ephemeral: true });
                    this.used--;
                    return AbilityResponse.FAILURE;
                };
                myStats.activeIIused++;
                notice.push(`\n✨ I LOVE LOVE LOVE LOVE YOU!`);
                dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `✨ **${char.name}**'s LOVE BEAM`, { atkMultiplier: 0.01 * (myStats.ce / 8) });
                myStats.ce -= Math.min(myStats.ce, 1000);
            };
            return AbilityResponse.SUCCESS;
        },
        passive: async function (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) {
            ...
            myStats.activeIIused = 0;
            myStats.ignoreRestrain = false;
            myStats.cskillpause = -11;
            myStats.cskillcombo = 0;

            notice.push(`\n<a:ceanimated:1468216281768661093> **${char.name}** will store and manipulate Cursed Energy [ <:ce:1466817050860191817> ]...`);

            myStats.replaceButton.cskill = {
                "emoji": "☠️",
                "run": async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    if (myStats.cskillpause === matchStats.round + 10 && this.spskill === -1) {
                        noTimeout(matchStats, myStats);
                        this.spskill = 2;
                        delete myStats.replaceButton.atk;
                        delete myStats.replaceButton.def;
                        delete myStats.replaceButton.ability;

                        notice.push(`\n⚜️ **${char.name}** has specialized in __Cursed Speech__.`);
                        return AbilityResponse.SUCCESS;
                    };

                    if (myStats.ce / myStats.ceCap < 0.5 || myStats.cskillpause > matchStats.round) {
                        if (myStats.ce < 50) {
                            noTimeout(matchStats, myStats);
                            matchStats.interaction.followUp({ content: `**${char.name}** does not have sufficient Cursed Energy (${myStats.ce}/50)`, ephemeral: true });
                            return AbilityResponse.FAILURE;
                        } else {
                            myStats.ce -= 50;
                            let techIndex = (this.spskill !== -1) ? this.spskill : Math.floor(Math.random() * 2);
                            switch (techIndex) {
                                case 0: // Shrine
                                    eStats.dodge -= 0.25;
                                    if (eStats.dodge < 0) eStats.dodge = 0;
                                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `☠️ Shrine! **${char.name}**`, { atkMultiplier: 0.3 });
                                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `☠️ Shrine! **${char.name}**`, { atkMultiplier: 0.3 });
                                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `☠️ Shrine! **${char.name}**`, { atkMultiplier: 0.3 });
                                    break;
                                case 1: // Thin Ice Breaker
                                    eStats.vulnerabilityDynamic += 0.05;
                                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `☠️ **${char.name}** casts Thin Ice Barrier. He`, { atkMultiplier: 0.7 + Math.random() * (0.2) });
                                    myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 1, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                                        eStats.vulnerabilityDynamic -= 0.05;
                                        return AbilityResponse.SUCCESS;
                                    }));
                                    break;
                                case 2: // Cursed Speech
                                    eStats.atk -= Math.floor(eStats.atk * 0.25);
                                    eStats.md -= Math.floor(eStats.md * 0.25);
                                    eStats.cr -= 0.25;
                                    if (eStats.cr < 0) eStats.cr = 0;
                                    eStats.cd -= 0.25;
                                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `☠️ Stop. **${char.name}**`, { atkMultiplier: 0.2 });
                                    break;
                            };
                        };
                    } else {
                        noTimeout(matchStats, myStats);
                        if (myStats.cskillpause > matchStats.round) {
                            matchStats.interaction.followUp({ content: `**${char.name}** needs to rest ${this.pause - matchStats.round} more ${this.pause - matchStats.round === 1 ? "round" : "rounds"} before entering his domain.`, ephemeral: true });
                            return AbilityResponse.FAILURE;
                        };

                        myStats.cskillpause = matchStats.round + 10;
                        const tempAtk = myStats.replaceButton.atk;
                        const tempDef = myStats.replaceButton.def;
                        const tempAbility = myStats.replaceButton.ability;

                        notice.push(`\n⚜️ **${char.name}** entered the domain -- __[Authentic Mutual Love (真贋相愛)]__ for **7** rounds. Use ${tempAtk?.emoji || "⚔️"} / ${tempDef?.emoji || "🛡️"} / ⚜️ to specialize in a skill.`);

                        myStats.replaceButton.atk = {// Shrine
                            emoji: tempAtk?.emoji,
                            run: async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                                noTimeout(matchStats, myStats);
                                this.spskill = 0;
                                ...
                                notice.push(`\n⚜️ **${char.name}** has specialized in __Shrine__.`);
                                return AbilityResponse.SUCCESS;
                            },
                        };

                        myStats.replaceButton.def = {// Thin Ice Breaker
                            emoji: tempDef?.emoji,
                            run: async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                                noTimeout(matchStats, myStats);
                                this.spskill = 1;
                                ...
                                notice.push(`\n⚜️ **${char.name}** has specialized in __Thin Ice Breaker__.`);
                                return AbilityResponse.SUCCESS;
                            },
                        };
                        ...
                        myStats.replaceButton.ability = {
                            ...
                            run: async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                                noTimeout(matchStats, myStats);
                                notice.push(`\n⚜️ **${char.name}** has yet to pick the skill to specialize in.`);
                        ...
                        myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 7, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                            dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `⚜️ **${char.name}** casts [Max. Output - Jacob's Ladder] and`, { atkMultiplier: 0.1 * Math.min(myStats.cskillcombo, 7) });
                            this.spskill = -1;
                            myStats.cskillcombo = 0;
            ...
            myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                let manaConvert = Math.floor(myStats.sm * 0.8);
                myStats.sm -= manaConvert;
                myStats.ce += manaConvert;
                if (myStats.ce > myStats.ceCap) myStats.ce = myStats.ceCap; // Cap at 2000 by default
                notice.push(`\n<a:ceanimated:1468216281768661093> ${char.name} now has ${myStats.ce} <:ce:1466817050860191817>`);

                // ATK +1% for every 200 CE, up to 10%
                //myStats.atk += Math.floor(myStats.atk * Math.min(0.01 * Math.round(myStats.ce / 200), 0.1));

                // Heal 5% max HP with 100 CE when below 40% HP
                //if (myStats.hp / myStats.maxhp < 0.4) addHeal(myStats, eStats, myStats, mybuff, ebuff, matchStats, notice, ``, Math.floor(myStats.maxhp * 0.05), {});
                return AbilityResponse.SUCCESS;
            }, 9999));
    ...
    "25977": {
        ...
        cost: 0,
        pause: -8,
        desc: "**Total Usage**: `Unlimited (CD: 8)`\n**Cost**: `140 💧 (reduced by 20 every use, can be substituted with 25% max HP 💖)`\n**Timeout**: `False`\n**Tags**: `DPS (Flexible ATK/MD, DPS/Tank)`\n\n-# Fern and Stark together are not only bickering allies, but also a mix of both physical and magical force, tearing down waves of enemies with their collaborative efforts.\n\nPassively, `Fern` enters the battle as the front-liner if MD is higher than ATK, else `Stark` enters. This activates their [Upon entry] effect respectively. When either member is [On-field], the other activates their [Off-field] passive. Upon being switched to, activates their own [Upon entry] effect.\n\n`Fern`'s ability:\n- ATK is altered to Zoltraak, dealing **25%** magical damage. If she has **15** :droplet:, spends it to follow-up with **25%** MD, up to **3** times per ATK action. When [On-field], she has **+200** MR. When [Off-field], she has a **1%** chance to detect mana-instability for every **1** mana-regen, granting `Stark` **+4%** additional mitigation against physical damage, and another **+8%** mitigation against magical damage as well. [Upon entry], she inflicts **+5%** vulnerability on the enemy for **8** rounds, after that she gains **+20%** vulnerability.\n\n`Stark`'s ability:\n- ATK is altered. First four ATKs deal **60%** physical damage and allows him to store the mitigated amount. The **8th** ATK deals the mitigated amount (Up to 20% max HP) to the enemy as *lightning* physical absolute damage, before resetting mitigation recorded. When [On-field], he gains **4%** mitigation against physical damage. When [Off-field], he grants **5** 💧 every round. [Upon entry], he has **+10%** ATK for **8** rounds, after that he has **-10%** ATK.\n\nUsing their active, they switch between `Fern`/`Stark`. Loses **2%** current HP every round permanently. If below **30%** HP, additionally launches a collaborative strike, dealing **40%** damage. For every time this active is used, lowers its cost by **20** (down to at most 0). On this round, passives of both characters are briefly applied.",
        shortdesc: "__**Passive**__:\n- Enters the battle as `Fern` if MD is higher than ATK, else enters as `Stark`. This activates their [Upon entry] effect.\n- When either member is [On-field], the other activates their [Off-field] passive. Upon being switched to, activates their own [Upon entry] effect.\n\n`Fern`:\n- ATK - Zoltraak: Deals **25%** magical damage. If she has **15** :droplet:, spends it to follow-up with **25%** MD, up to **3** times per ATK action.\n- [On-field] : **+200** MR\n- [Off-field] : For every **1** mana regen, has a **1%** chance to detect mana-instability, allowing Stark to gain **+4%** additional mitigation against physical damage, and another **+8%** mitigation against magical damage as well.\n- [Upon entry] : **+5%** vulnerability on the enemy for **8** rounds, after that she gains **+20%** vulnerability.\n\n`Stark`:\n- ATK: First **7** ATKs deal **60%** physical damage and allow him to store the mitigated amount. The **8th** ATK deals the mitigated amount (Up to 20% max HP) to the enemy as *lightning* physical absolute damage, before resetting mitigation recorded.\n- [On-field] : Gains **4%** mitigation against physical damage.\n- [Off-field] : Grants **+5** 💧 every round.\n- [Upon entry] : Has **+10%** ATK for **8** rounds, after that he has **-10%** ATK.\n\n__**Active** (:sparkles:)__:\nUses: `Unlimited (CD: 8)`\nCost: `140 💧 (can be substituted with 25% max HP, at most dropping HP to 1)`\nTimeout: `False`\n- Switches between `Fern`/`Stark`.\n- Loses **2%** current HP every round permanently\n- If below **30%** HP, additionally launches a collaborative strike, dealing **40%** damage.\n- For every time this active is used, lowers its cost by **20** (down to at most 0). On this round, passives of both characters are briefly applied.",
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            // Fern & Stark EX
            ...
            if (this.pause > matchStats.round) {
                noTimeout(matchStats, myStats);
                this.used--;
                matchStats.sendWarning({ content: `**${char.name}** needs to prepare for ${this.pause - matchStats.round} more ${this.pause - matchStats.round === 1 ? "round" : "rounds"}`, ephemeral: true });
                return AbilityResponse.FAILURE;
            };

            // Lost HP or return if insufficient cost
            let currCost = Math.max(140 - (20 * this.used), 0);
            if (myStats.sm < currCost) {
                myStats.hp -= Math.floor(myStats.maxhp * 0.25);
            ...
            // Remove any overdue debuffs
            if (myStats.onFieldFor > 5) {
                if (myStats.onField === 1) {
                    myStats.vulnerabilityDynamic -= 0.2;
                } else {
                    myStats.atk += Math.floor(myStats.atk * 0.1);
                };
            };
            ...
            // Switch char
            myStats.onField = myStats.onField === 1 ? 2 : 1; // 1 = Fern, 2 = Stark
            myStats.onFieldFor = 1;
            myStats.atkCount = 0;

            // Upon-entry effects
            if (myStats.onField === 1) {
                notice.push(`\n✨ Mhmm. Engaging the enemy.`);
                eStats.vulnerabilityDynamic += 0.1;
            } else {
                notice.push(`\n✨ That's what a vanguard's for!`);
                //myStats.counter++;
                myStats.atk += Math.floor(myStats.atk * 0.1);
            };
            notice.push(`\n🔁 **${char.name}** switched on-field and off-field spots.`);

            // Apply HP debuff
            mybuff.hp.push(new buffInfo("*", 0.98, 9999));

            // Deal damage if below 30% HP
            if (myStats.hp / myStats.maxhp < 0.3) dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `✨ **${char.name}** landed a collaborative strike! They`, { atkMultiplier: 0.4 });

            return AbilityResponse.SUCCESS;
        },
        passive: async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
            myStats.onField = myStats.md > myStats.atk ? 1 : 2; // 1 = Fern, 2 = Stark
            myStats.onFieldFor = 1; // Record how long the char has stood on field
            eStats.vulnerabilityDynamic ??= 1;
            myStats.vulnerabilityDynamic ??= 1;
            //myStats.counter ??= 0;
            myStats.physMit ??= 0;
            myStats.mdMit ??= 0;
            myStats.mitrecord = false;
            myStats.mitstore = 0;
            myStats.atkCount = 0;
            ...
            if (myStats.onField === 1) {
                notice.push(`\n✨ Mhmm. Engaging the enemy.`);
                eStats.vulnerabilityDynamic += 0.02;
                myStats.mr += 200; // Fern's on-field effect
            } else {
                notice.push(`\n✨ That's what a vanguard's for!`);
                //myStats.counter++;
                myStats.atk += Math.floor(myStats.atk * 0.1);
            ...
            myStats.replaceButton.atk = {
                ...
                run: async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    if (myStats.onField === 1) {
                        dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `🪄 **Fern** used Zoltraak and dealt`, { atkMultiplier: 0.25, magicDamage: true });
                        while (myStats.sm >= 15) {
                            myStats.sm -= 15;
                            dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `🪄 **Fern** followed up and dealt`, { atkMultiplier: 0.25, magicDamage: true });
                            myStats.atkCount++;
                            if (myStats.atkCount === 3) break;
                        };
                    } else {
                        myStats.atkCount++;
                        if (myStats.atkCount % 8 !== 0) {
                            myStats.mitrecord = true;
                            dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `⚔️ **Stark** dealt`, { atkMultiplier: 0.6, magicDamage: false });
                        } else {
                            dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `<:lightning:1340309243827458139> Sentengeki! **Stark**`, { overwriteDamage: Math.floor(Math.min(myStats.mitstore, myStats.maxhp * 0.2)), isLightning: true, magicDamage: false });
                            myStats.mitstore = 0;
                        };
                    };
                    return AbilityResponse.SUCCESS;
            ...
            myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                myStats.onFieldFor++;
                if (myStats.onField === 1) myStats.atkCount = 0;
                myStats.mitrecord = false;
                // Check for entry buff expiry + debuff self
                if (myStats.onField === 1 && myStats.onFieldFor === 9) {
                    eStats.vulnerabilityDynamic -= 0.02;
                    myStats.vulnerabilityDynamic += 0.2;
                } else if (myStats.onField === 2 && myStats.onFieldFor > 8) {
                    myStats.atk -= Math.floor(myStats.atk * 0.1);
                };

                // Check what on & off-field passives to apply
                if (myStats.onField === 1) {
                    ...
                    myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 1, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                        myStats.physMit -= 0.04;
                        return AbilityResponse.SUCCESS;
                    }));
                    if (Math.random() < 0.01 * myStats.mg) { // Fern's off-field passive
                        myStats.physMit += 0.04;
                        myStats.mdMit += 0.08;
                        notice.push(`\n✨ **Fern** granted **Stark** damage mitigation`);
                        myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 1, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                            myStats.physMit -= 0.04;
                            myStats.mdMit -= 0.08;
                            return AbilityResponse.SUCCESS;
    ...
    "26200": {
        ...
        pause: 0,
        cost: 100,
        desc: "**Total Usage**: `Unlimited (CD: 8)`\n**Cost**: `100 💧`\n**Timeout**: `False`\n**Tags**: `DPS`\n\n-# Kuutar, the Moon Goddess of Nod-Krai.\n-# The Damselette.\n-# She did not recognize herself in those reflections. Call her, *Columbina Hyposelenia*.\n\nAt the outset, she increases the base damage scaling of Lunar instances by **0.25%** for every **1000** max HP (max: 10%)\n\nHer Normal ATK is altered to summon Moonlit Tides, deals **80%** undodgeable DMG, before gaining **1** `Verdant Dew`. When **5** `Verdant Dew` are present, they are consumed to trigger an instance of lunar reaction (random out of the three):\n> - Lunar-Charged: Deals **20%** lightning damage and recovers **3** mana.\n> - Lunar-Bloom: Next Lightning strike deals **40%** more damage (stackable).\n> - Lunar-Crystallize: Gains a shield, based on **2.5%** of her max HP (Max: 5000).\n\nUsing her active: Moonlit Melancholy, hills and tides unite under the silver light of a pristine new moon. For **6** rounds, transforms the surrounding terrain into __Lunar Domain__. While in the domain, consuming `Verdant Dew` triggers all **3** Lunar reactions simultaneously, with them gaining the following buffs:\n> - Lunar-Charged: Lunar Charged has a **33%** chance to twinshot\n> - Lunar-Bloom: Gain an additional `Verdant Dew`, and buff Lightning damage for **+3** instances.\n> - Lunar-Crystallize: Deals **40%** physical damage\n\nIn a party, she triggers **1** random Lunar Reaction every **5** rounds.",
        shortdesc: "**Uses**: `Unlimited (CD: 8)`\n**Cost**: `100 💧`\n**Timeout**: `False`\n\n__**Passive**__:\n- For every **1000** max HP, Lunar Reactions deal **0.25%** more damage (max: **10%**)\n- Normal attack is altered:\n> - Deal **80%** undodgeable DMG and gains **1** `Verdant Dew`.\n> - When **5** `Verdant Dew` are present, consume them to trigger **1** Lunar Reaction out of the 3 randomly.\n\n__Core Mechanic: Lunar Reaction__\n- Lunar-Charged: Deals **20%** lightning damage and recovers **3** :droplet:.\n- Lunar-Bloom: Next lightning strike deals **40%** more damage (stackable)\n- Lunar-Crystallize: Gains a shield, based on **2.5%** of her Max HP (Max: **5000**).\n\n__**Active (:sparkles:)**__:\n- Enters `Lunar Domain` for **6** rounds:\n-  All **3** Lunar Reactions simultaneously, and gain the following buffs:\n> - Lunar-Charged: Lunar Charged has a **33%** chance to twinshot\n> - Lunar-Bloom: Gain an additional `Verdant Dew`, and buff Lightning damage for **+3** instances.\n> - Lunar-Crystallize: Deals **40%** physical damage\n\n__**Party (:busts_in_silhouette:)**__:\n- Triggers **1** random Lunar Reaction every **5** rounds.",
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            // Columbina EX
            ...
            myStats.lunarDomain = true;

            myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 6, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                myStats.lunarDomain = false;

                return AbilityResponse.SUCCESS;
            }));
            notice.push(`\n<a:columult:1480218976184176847> **${char.name}** entered __Lunar Domain__ for **6** rounds`);

            return AbilityResponse.SUCCESS;
        },
        passive: async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
            myStats.verdantDew = 0;
            myStats.lunarDomain = false;
            myStats.tempLightningBuff ??= 0;
            myStats.tempLightningBuffActive ??= 0;
            myStats.replaceButton.ability = { "emoji": "<a:columult:1480218976184176847>" };
            myStats.replaceButton.atk = {
                emoji: "<a:columatk:1485117705307557938>",
                run: async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                    dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `<a:columatk:1485117705307557938> **${char.name}**`, { atkMultiplier: 0.8, magicDamage: true, dodge: false });
                    myStats.verdantDew++;
                    if (myStats.verdantDew >= 5) {
                        // Trigger Lunar Reaction
                        myStats.verdantDew -= 5;
                        const atkbuff = Math.min(0.0025 * Math.round(myStats.maxhp / 1000), 0.1);
                        let lunarIndex = Math.floor(Math.random() * 3);
                        if (myStats.lunarDomain) lunarIndex = 5;
                        if (lunarIndex === 0 || lunarIndex === 5) {// Lunar-Charged
                        ...
                        if (lunarIndex === 1 || lunarIndex === 5) {// Lunar Bloom
                            myStats.tempLightningBuff += 0.4;
                            myStats.tempLightningBuffActive += myStats.lunarDomain ? 3 : 1;
                            if (myStats.lunarDomain) myStats.verdantDew++;
                            notice.push(`\n<a:columult:1480218976184176847> **Lunar Bloom** boosted lightning damage for **${myStats.lunarDomain ? 3 : 1}** ${myStats.lunarDomain ? "instances" : "instance"}`);
                            myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 1, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                                myStats.tempLightningBuff -= 0.4;
                        ...
                        if (lunarIndex === 2 || lunarIndex === 5) {// Lunar Crystallize
                            ...
                            if (myStats.lunarDomain) {
                                dealDamage(eStats, myStats, ebuff, mybuff, matchStats, notice, `<a:columult:1480218976184176847> **Lunar-Crystallize**`, { atkMultiplier: 0.4 + atkbuff, magicDamage: false });
                            };
                        };
                    };
                    return AbilityResponse.SUCCESS;
        ...
        party: async (pStats, myStats, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
            myStats.tempLightningBuff ??= 0;
            myStats.tempLightningBuffActive ??= 0;
            // Trigger Lunar Reaction every 5 rounds
            myStats.delayedBuffs.push(new delayedBuffs(0, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                if (matchStats.round % 5 !== 0) return AbilityResponse.FAILURE;
                let lunarIndex = Math.floor(Math.random() * 3);
                if (lunarIndex === 0) {// Lunar-Charged
                    ...
                    notice.push(`\n<a:columult:1480218976184176847> **Lunar Bloom** boosted lightning damage for **1** instance`);
                    myStats.delayedBuffs.push(new delayedBuffs(matchStats.round + 1, async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
                        myStats.tempLightningBuff -= 0.4;

                        return AbilityResponse.SUCCESS;
                    }));
                } else if (lunarIndex === 2) {// Lunar Crystallize
                    let shieldGain = Math.min(Math.round(myStats.maxhp * 0.025), 5000);
                    myStats.shield += shieldGain;
                    notice.push(`\n<a:columult:1480218976184176847> **Lunar Crystallize** granted ${char.name} **${shieldGain}** 💠`);
                };
    ...
    "26272": {
        ...
        pause: 0,
        cost: 0,
        desc: "**Total Usage**: `Unlimited`\n**Cost**: `600 Frostheart / None`\n**Timeout**: `True`\n**Tags**: `DPS`\n\n-# A veteran miko (shrine maiden) and member of the Special Response Force who has served for over a century, Hiyuki wields the Suzu, allowing her to perceive and harness endless *possibilities* of herself.\n\nUnder glacio synergy, all elementless attacks in battle are identified as <a:frost1:1504497507185725473><a:frost2:1504497408233705512>, and apply **1x** Glacio Chafe (considered as Frost [`❄️`]). When the enemy has `❄️` and is attacked by a Frost hit, they take **4%** damage for every stack (Max: 40%, once per round).\n\nUpon entering battle, she uses ATK on the enemy **twice** (procs normal attack effects). Every ATK grants **50** `Dedication`, while every **3rd** grants **200** `Dedication`. When owning **300** of such, ATK instead consumes **300** to deal **50%** damage with no timeout, before entering her __Foreclaimed self__ state (once).\n\n-# Manipulation of parallel dimensions and alternative timelines.\nDuring the __Foreclaimed self__ state, using ATK or class skill grants **100** `Frostheart` (Max: 900). She has **+20%** counter chance, normal attacks also deal **120%** damage. When reaching **600** or more `Frostheart`, she may use :sparkles:, consuming **600** to extend all buff durations on self by **1** round, counter the next hit, and enter the __Iai state__ (once)\n\n-# The absolute convergence of all parallel timelines into a single, fatal moment of execution.\nDuring the __Iai__ state, her next three ATKs are altered to deal **150%** undodgeable damage and apply **3x** `❄️`. If the enemy has **10** or more `❄️`, consumes it to freeze them for **1** round. When all three are used up, she may use :sparkles: again to deal **400%** undodgeable damage, converting any `Frostheart` to `Dedication`, then exits the __Iai__ state.",
        shortdesc: "**Uses**: `Unlimited`\n**Cost**: `600 Frostheart / None`\n**Timeout**: `True`\n**Role**: `DPS (Frost), progressive, combo`\n\n__Core Mechanic: Glacio Synergy__\n- All of her + her allies' attacks, if they don't carry an element (e.g. Fire / Lightning), are identified as <a:frost1:1504497507185725473><a:frost2:1504497408233705512>, and apply **1x** Glacio Chafe (considered as Frost [`❄️`]).\n- `❄️` : when attacked with a Frost hit, deals **4%** damage for every stack, up to **40%** (once per round)\n\n__Default state__\n- Upon entering battle, uses ATK on the enemy **twice**\n- Every ATK grants **50** `Dedication`, while every third grants **200** `Dedication`\n- When owning **300** `Dedication`, ATK instead consumes **300** `Dedication` to deal **50%** damage with no timeout, before entering her __Foreclaimed self__ state (once)\n\nDuring the __Foreclaimed self__ state:\n- Using ATK or Class skill grants **100** `Frostheart` (Max: 900)\n- has **+20%** counter chance, normal attacks deal **+20%** damage\n- When reaching **600** `Frostheart`, :sparkles: becomes available (Timeout true). Using her active consumes **600** `Frostheart` to extend all buff durations on self by **1** round, counter the next hit, and enter __Iai__ state (once)\n\nDuring __Iai__ state:\n- Next three ATK are altered to deal **150%** undodgeable damage and apply **3x** `❄️`. If the enemy has **10** or more `❄️`, consumes it to freeze them for **1** round\n- When all three are used up, :sparkles: becomes available again (Timeout true). Using her active deals **400%** undodgeable damage, and converts any `Frostheart` to `Dedication`, then exits the __Iai__ state.",
        ability: async function (myStats, myStatsFixed, eStats, eStatsFixed, mybuff, ebuff, char, enemy, matchStats, notice, embed, message, ...list) {
            // Hiyuki EX
    ...
    "26273": {
        ...
        passive: async (myStats, myStatsFixed, eStats, mybuff, ebuff, char, enemy, matchStats, notice, embed, user, ...list) => {
            ...
            if (myStats.manaGained >= (circlereq * (myStats.manaCircle + 1))) {
                myStats.manaCircle++;
                myStats.atk += Math.floor(myStats.atk * 0.12);
                mybuff.atk.push(new buffInfo("*", 1.12, 9999));
